/** Minimal UTF-8 DXF (AC1027 / 2013). 1 unit = 1 mm. */

type Align = "left" | "center" | "right";

type LineEnt = { x1: number; y1: number; x2: number; y2: number; layer: string };
type CircEnt = { x: number; y: number; r: number; layer: string };
type TextEnt = { x: number; y: number; h: number; text: string; align: Align; layer: string; bold?: boolean };
type ArcEnt = { x: number; y: number; r: number; start: number; end: number; layer: string };
...
export class DxfDoc {
  ...
  toString(): string {
    ...
    for (const e of this.lines) {
      chunks.push(
        p(0, "LINE", 5, nextH(), 100, "AcDbEntity", 8, e.layer, 100, "AcDbLine", 10, f(e.x1), 20, f(flip(e.y1)), 30, 0, 11, f(e.x2), 21, f(flip(e.y2)), 31, 0),
      );
    }
    for (const e of this.circles) {
      chunks.push(p(0, "CIRCLE", 5, nextH(), 100, "AcDbEntity", 8, e.layer, 100, "AcDbCircle", 10, f(e.x), 20, f(flip(e.y)), 30, 0, 40, f(e.r)));
    }
    for (const e of this.arcs) {
      chunks.push(
        p(0, "ARC", 5, nextH(), 100, "AcDbEntity", 8, e.layer, 100, "AcDbArc", 10, f(e.x), 20, f(flip(e.y)), 30, 0, 40, f(e.r), 50, f(e.start), 51, f(e.end)),
      );
    }
